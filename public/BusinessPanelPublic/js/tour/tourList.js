    ...
    $.ajax({
        ...
        success: (response) => {
            if (response.status == "ok") {
                ...
                createTourList();
            } else showSuccessNotifiBP("Server Error", "left", "red");
        },
        error: (err) => {
...
function createEditModalBody(list) {
    ...
    list.forEach((item) => {
        ...
        if (inTour.remainingStage != null) {
            inTour.remainingStage.map(
                (e) => (minOfStage = minOfStage > e ? e : minOfStage)
            );
        ...
        html += `<div class="editCard card${item.stage} ${
            canEdit ? "" : "notAllow"
        }" onclick="goToEditPage(${item.stage})">
...
function goToEditPage(_stage) {
    ...
    if (canEdit)
        openLoading(
            false,
            () =>
                (location.href = `${editTourUrl}/stage_${_stage}/${tours[openEditIndex].id}`)
        );
    else {
        openErrorAlertBP(
            "برای ویرایش این بخش ابتدا باید مراحل قبل را تکمیل کنید."
        );
    }
}
...
function deleteTour(_index) {
    ...
    openWarningBP(
        "ایا از حذف تور اطمینان دارید؟ در صورت حذف امکان بازگشت تور وجود ندارد",
        doDeleteTour,
        "بله، پاک شود"
    );
}
...
function doDeleteTour() {
    openLoading(false, () => {
        $.ajax({
            ...
            success: (response) => {
                if (response.status == "ok") getTourList();
                else if (response.status == "registered") {
                    closeLoading();
                    openErrorAlertBP(
                        "برای تور شما افرادی ثبت نام کرده اند و شما نمی توانید تور خود را حذف کنید."
                    );
