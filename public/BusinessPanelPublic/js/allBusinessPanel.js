...
function validateDate(evt, inputId) {
    ...
    if (key != 8) {
        ...
        inputDate += key;
    } else inputDate = inputDate.substring(0, inputDate.length - 1);
    ...
    switch (inputDate.length) {
        ...
        case 5:
            output =
                inputDate.substring(0, 4) + " / " + inputDate[4] + "_ / __";
            break;
        case 6:
            output =
...
function resizeFitImg(_class) {
    let imgs = $("." + _class);
    for (let i = 0; i < imgs.length; i++) fitThisImg(imgs[i]);
}
...
function isNeedSideProgressBar(_kind) {
    if (_kind) $("#mainSideBar").addClass("hasSideProgressBar");
    else $("#mainSideBar").removeClass("hasSideProgressBar");
}
...
function openWarningBP(_text, _callBack = false, _okText = "بسیار خب") {
    ...
    else $("#warningModalCallBackShowBP").hide();
...
function closeWarningBP() {
    if (
        alertWarningCallBack !== false &&
        typeof alertWarningCallBack === "function"
    )
...
function closeErrorAlertBP() {
    ...
    if (typeof errorAlertCallBack === "function") errorAlertCallBack();
}
