...
$(document).ready(function () {
    ...
    dropArea.addEventListener(
        "dragenter",
        () => dropAreaJQ.addClass("highlight"),
        false
    );
    dropArea.addEventListener(
        "dragleave",
        () => dropAreaJQ.removeClass("highlight"),
        false
    );
    dropArea.addEventListener(
        "dragover",
    ...
    dropArea.addEventListener(
        ...
        (e) => {
            ...
            if (files.length > 0)
                createAndUploadFilePic(
                    files[0],
                    "additionalValue",
                    "showUploadPicsSection"
                );
        },
        false
    );
...
function createSearchInput(_doFuncName, _placeHolderTxt) {
    ...
    $("#globalSearchInput")
        .attr("onkeyup", _doFuncName + "(this)")
        .attr("placeholder", _placeHolderTxt)
        .val("")
        .focus();
}
...
function searchCity(_element) {
    ...
    if (value.trim().length > 1) {
        if (citySearchAjax != null) citySearchAjax.abort();
...
function checkStepOne() {
    ...
    if (!data.haghighi && !data.hoghoghi) {
        showSuccessNotifiBP(
            "لطفا نوع ارائه دهنده خود را مشخص کنید.",
            "right",
            "#ac0020"
        );
        return false;
...
function checkStepTwo() {
    if (data.type === "None") {
        showSuccessNotifiBP(
            "لطفا نوع خدمت قابل ارائه خود را مشخص کنید.",
            "right",
            "#ac0020"
        );
        return false;
...
function checkStepThree(progress) {
    ...
    if (
        ...
        showSuccessNotifiBP(
            "لطفا تمام فیلد ها را به شکل صحیح پر نمایید.",
            "right",
            "#ac0020"
        );
        return false;
    ...
    if (created) requestUrl = updateBusinessInfo1BaseUrl + "/" + data.id;
...
function checkStepFour(progress) {
    ...
    if (data.type != "tour") {
        if (
            data.closedDayStart.length == 0 ||
            data.closedDayEnd.length == 0 ||
            data.afterClosedDayStart.length == 0 ||
            data.afterClosedDayEnd.length == 0 ||
            data.inWeekDayStart.length == 0 ||
            data.inWeekDayEnd.length == 0
        ) {
            showSuccessNotifiBP(
                "لطفا تمام فیلد ها را به شکل صحیح پر نمایید.",
                "right",
...
function checkStepFive(progress) {
    ...
    for (i = 0; i < uploaders.length; i++) {
        ...
        if (name.length == 0) {
            showSuccessNotifiBP(
                "لطفا نام عضو " + (idx + 1) + " را وارد نمایید.",
                "right",
                "#ac0020"
            );
            return false;
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            if (res.status === "ok") {
                ...
                currentPage++;
                $("#step" + currentPage).removeClass("hidden");
            } else showSuccessNotifiBP(res.msg, "right", "#ac0020");
        },
        error: function (reject) {
            errorAjax(reject);
        },
...
function checkFinalStep() {
    if (!$("#confirmContract").prop("checked")) {
        showSuccessNotifiBP(
            "تایید متن قرارداد همکاری الزامی است.",
            "right",
            "#ac0020"
        );
        return;
...
function goToPage(_step, progress) {
    ...
    if (
        (currentPage == 1 && _step == -1) ||
        (currentPage == 3 && _step == -1 && mode == "edit")
    )
        return;
    else if (currentPage == 1 && !checkStepOne()) return;
    else if (currentPage == 2 && _step > 0 && !checkStepTwo()) return;
    else if (currentPage == 3 && _step > 0 && !checkStepThree(progress)) return;
    else if (currentPage == 4 && _step > 0 && !checkStepFour(progress)) return;
    else if (currentPage == 5 && _step > 0 && !checkStepFive(progress)) return;
    else if (currentPage == 6 && _step > 0 && !checkStepSix(progress)) return;
    else if (currentPage == 7 && _step > 0 && !checkStepSeven()) return;
...
function initUploader(field, showSection, textSection, max) {
    ...
    if (idx != -1) {
        if (max <= uploaders[idx][4].length) {
            showSuccessNotifiBP(
                "در این قسمت حداکثر " + max + " فایل قابل آپلود است.",
                "right",
                "#ac0020"
            );
            return -1;
...
function createAndUploadFilePic(_files, idx) {
    if (uploaders[idx][4].length >= uploaders[idx][3]) return;
...
function createNewImgUploadCard(_index, idx) {
    ...
    var text = `<div id="uplaodedImg_${file.code}" class="uploadFileCard">
                    ...
                    <div id="warningUpload_${
                        file.code
                    }" class="absoluteBackground warningIcon">اشکال در بارگذاری</div>
                    <div class="absoluteBackground process">
                        <div class="lds-ring"><div></div><div></div><div></div><div></div></div>
                        <div class="processCounter">0%</div>
                    </div>
                    <div class="hoverInfos" style="cursor: pointer">
                        <div onclick="deleteThisUploadedImage(${
                            file.code
                        },'${getIdxInFitImages(
        uploaders[idx][0]
    )}')" class="cancelButton closeIconWithCircle"> حذف عکس </div>
                    </div>
                </div>`;
...
function deleteThisUploadedImage(_code, idx) {
    ...
    if (index != null) {
        if (uploaders[idx][4][index].uploaded == 0) cancelThisImg = true;
        else if (
            uploaders[idx][4][index].uploaded == -1 ||
            uploaders[idx][4][index].uploaded == -2
        ) {
...
function doDeletePicServer(_index, idx) {
    ...
    } else formData["field"] = uploaders[idx][0];
    ...
    $.ajax({
        ...
        success: (response) => {
            ...
            } else showSuccessNotifiBP(response.msg, "right", "#ac0020");
        },
        error: function (reject) {
            errorAjax(reject);
        },
...
function uploadQueuePictures(idx) {
    if (uploadProcess == false) {
        ...
        uploaders[idx][4].map((item, index) => {
            if (item.uploaded == -1 && uploadIndex == null) uploadIndex = index;
        });
        ...
        if (uploadIndex != null) {
            ...
            } else formData.append("field", uploaders[idx][0]);
            ...
            uploadPicAjax = $.ajax({
                ...
                success: (response) => {
                    uploadProcess = false;
                    if (response.status == "ok") {
                        if (madarek_idx != -1) id_idx[madarek_idx] = 1;
...
function changeAdditionalValue() {
    ...
    if (data.hasAdditionalValue) $(".additionalValue").removeClass("hidden");
    else $(".additionalValue").addClass("hidden");
}
