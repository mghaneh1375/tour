...
function fillInputs(){
    ...
    storeData.lastDays.map((item, index) => {
        if($('#dayDiscountInput_'+index).length == 0)
            addLastDayDiscount()
...
function checkInput(_mainStore = true){
    ...
    if (errorText == '' || _mainStore == false) {
        ...
        if (warning == '' && _mainStore)
            ...
            openWarningBP(`<ul>${warning}</ul>`, doSaveInfos, 'مشکلی نیست');
    }
    else
        openErrorAlertBP(`<ul>${errorText}</ul>`);
...
function createNewPriceRow(){
    ...
    var priceHtml = `<div class="tourBasicPriceTourCreation tourOtherPrice col-md-12">
                        ...
                        <div class="deleteButton" onclick="deleteThisPrice(this)">
