...
Calendar.setup = function (params) {
	...
	param_default("_MIN_", null);
	param_default("_MAX_", null);
	...
	if(params.date != null) {
		miladi = JalaliDate.jalaliToGregorian(1396, 1, 11);
		params.date = new Date(miladi[0], miladi[1], miladi[2]);
	}
	...
	if(params._MIN_ != null)
		params._MIN_ = params._MIN_.replace(/\//g, '');

	if(params._MAX_ != null)
		params._MAX_ = params._MAX_.replace(/\//g, '');
	...
	function onSelect(cal) {
		...
		if (update && p.inputField) {
			if(params._MIN_ != null && params._MAX_ != null) {
				allow = false;
				selectedDate = cal.date.print(cal.dateFormat, p.ifDateType || cal.dateType, cal.langNumbers);
				selectedDate = selectedDate.replace(/\//g, '');
				if(selectedDate >= params._MIN_ && selectedDate <= params._MAX_) {
					$("#errorText").empty();
				}
				else {
					$("#errorText").empty();
					$("#errorText").append("تاریخ شما در بازه ی سفر شما قرار نمی گیرد");
					return;
				}
			}
