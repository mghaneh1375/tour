...
function searchForDestination(_element) {
    ...
    if (value.length > 1 && stateId != 0) {
        ...
        var url = kind == 'city' ? `${findCityWithStateUrl}?stateId=${stateId}&value=${value}` : `${findPlaceWithKindPlaceIdUrl}?value=${value}&kindPlaceId=6`;
...
function checkInput() {
    dataToSend = {
        ...
        tourDay: parseInt(document.getElementById('tourDay').value),
        tourNight: parseInt(document.getElementById('tourNight').value),
        minCapacity: parseInt(document.getElementById('minCapacity').value),
        ...
        userInfoNeed: [...document.querySelectorAll('input[name="userInfoNeed[]"]:checked')].map(item => item.value),
        dates: []
    };
    ...
    if (!dataToSend.anyCapacity) {
        if (!(Number.isInteger(dataToSend.minCapacity) && dataToSend.minCapacity >= 0))
            errorText += '<li>حداقل ظرفیت تور خود را مشخص کنید.</li>';
        if (!(Number.isInteger(dataToSend.maxCapacity) && dataToSend.maxCapacity >= 0))
